...
// provider already owns the "primary" added/modified/deleted keys, so ours render as
// "secondary" bars (VS Code's convention for a second active quick-diff source). We keep
// both sets in sync with our Explorer highlight color so the two visuals always match.
...
function resolveHighlightColor(): string {
  ...
  const customizations = workbenchConfig.get<Record<string, unknown>>('colorCustomizations') || {};
...
async function syncGutterColors(): Promise<void> {
  ...
  if (changed) {
    await workbenchConfig.update('colorCustomizations', updated, vscode.ConfigurationTarget.Global);
  }
}
...
function execGit(args: string[], cwd: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    cp.execFile('git', args, { cwd, maxBuffer: 1024 * 1024 * 20 }, (err: cp.ExecFileException | null, stdout: string, stderr: string) => {
      if (err) {
...
function toRelativePath(workspaceRoot: string, fsPath: string): string | undefined {
...
class BranchDiffDecorationProvider implements vscode.FileDecorationProvider {
  ...
  provideFileDecoration(uri: vscode.Uri): vscode.FileDecoration | undefined {
    ...
    const badge = vscode.workspace.getConfiguration(CONFIG_SECTION).get<string>('badge', 'B');
    const hasDiagnostics = vscode.languages
      .getDiagnostics(uri)
      .some((d) => d.severity === vscode.DiagnosticSeverity.Error || d.severity === vscode.DiagnosticSeverity.Warning);
  ...
  async refresh(workspaceRoot: string): Promise<void> {
    ...
    try {
      ...
      if (includeUncommitted) {
        const workingOut = await execGit(['diff', '--name-only', 'HEAD'], workspaceRoot);
  ...
  notifyChanged(uris: readonly vscode.Uri[]): void {
    const relevant = uris.filter((u) => u.scheme === 'file' && (this.committedFiles.has(this.normalize(u.fsPath)) || this.uncommittedFiles.has(this.normalize(u.fsPath))));
...
class BranchDiffContentProvider implements vscode.TextDocumentContentProvider {
  ...
  async provideTextDocumentContent(uri: vscode.Uri): Promise<string> {
    ...
    try {
      const mergeBase = (await execGit(['merge-base', base, 'HEAD'], this.workspaceRoot)).trim();
      return await execGit(['show', `${mergeBase}:${relativePath}`], this.workspaceRoot);
    } catch {
  ...
  invalidateOpenDocuments(): void {
    for (const doc of vscode.workspace.textDocuments) {
      ...
      this._onDidChange.fire(vscode.Uri.from({ scheme: ORIGINAL_SCHEME, path: '/' + relativePath }));
    }
  }
}
...
class BranchDiffQuickDiffProvider implements vscode.QuickDiffProvider {
  ...
  provideOriginalResource(uri: vscode.Uri): vscode.ProviderResult<vscode.Uri> {
    ...
    return vscode.Uri.from({ scheme: ORIGINAL_SCHEME, path: '/' + relativePath });
  }
}
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const statusBarItem = vscode.window.createStatusBarItem('branchDiffColors', vscode.StatusBarAlignment.Left, 100);
  ...
  sourceControl.quickDiffProvider = new BranchDiffQuickDiffProvider(workspaceRoot);
  ...
  context.subscriptions.push(vscode.window.onDidChangeActiveColorTheme(() => syncGutterColors()));
  ...
  context.subscriptions.push(
    vscode.languages.onDidChangeDiagnostics((e) => provider.notifyChanged(e.uris))
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('branchDiffColors.setBaseBranch', async () => {
      const config = vscode.workspace.getConfiguration(CONFIG_SECTION);
      ...
      if (input && input.trim()) {
        await config.update('baseBranch', input.trim(), vscode.ConfigurationTarget.Workspace);
      }
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('branchDiffColors.pickColor', async () => {
      const hex = await vscode.window.showInputBox({
        prompt: 'Hex color for files that differ from the base branch (e.g. #e2c08d)',
        placeHolder: '#e2c08d',
        validateInput: (v) => (/^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(v) ? undefined : 'Enter a valid hex color, e.g. #e2c08d')
      });
      if (!hex) return;
      ...
      vscode.window.showInformationMessage(`Branch Diff Colors: highlight color set to ${hex}`);
    })
  );
}
